...
public final class KeyboardUtils {
    ...
    public static void showSoftInput(@NonNull final View view, final int flags) {
        InputMethodManager imm =
                (InputMethodManager) App.instance.getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm == null) {
            return;
        }
    ...
    private static int getDecorViewInvisibleHeight(@NonNull final Window window) {
        ...
        if (delta <= Utils.INSTANCE.getNavBarHeight() + Utils.INSTANCE.getStatusBarHeight()) {
    ...
    private static int getContentViewInvisibleHeight(final Window window) {
        ...
        if (delta <= Utils.INSTANCE.getStatusBarHeight() + Utils.INSTANCE.getNavBarHeight()) {
            return 0;
        }
        return delta;
    }
